...
@section('title', 'Edit ' . $accessory->name . ' | BirdHaven Admin')
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            ...
            <h2 class="font-headline font-extrabold text-3xl text-on-surface">Edit Accessory</h2>
        </div>
    </div>
    ...
    <form action="{{ route('admin.accessories.update', $accessory) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div class="bg-surface-container-lowest rounded-lg shadow-sm border border-outline-variant/10 p-6">
                    ...
                    <div class="flex flex-col items-center gap-4">
                        ...
                        <p class="text-xs text-on-surface-variant text-center">Leave empty to keep current · max 2 MB</p>
                    </div>
                </div>
                ...
                <div class="bg-surface-container-lowest rounded-lg shadow-sm border border-outline-variant/10 p-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="col-span-2 space-y-2">
                            ...
                            <input name="name" type="text" value="{{ old('name', $accessory->name) }}" placeholder="e.g. Skyline Habitat Cage"
                        ...
                        <div class="col-span-2 space-y-2">
                            ...
                            <select name="type"
                                ...
                                @foreach($types as $value => $label)
                                    <option value="{{ $value }}" {{ old('type', $accessory->type) === $value ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-span-2 space-y-2">
                            ...
                            <textarea name="description" rows="4" placeholder="Describe the product..."
                                class="w-full px-4 py-3 bg-surface-container rounded-lg ring-1 ring-outline-variant/20 focus:ring-2 focus:ring-primary/20 outline-none transition-all text-sm resize-none">{{ old('description', $accessory->description) }}</textarea>
                        </div>
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                ...
                                <input name="price" type="number" step="0.01" min="0" value="{{ old('price', $accessory->price) }}"
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant text-sm">$</span>
                                <input name="original_price" type="number" step="0.01" min="0" value="{{ old('original_price', $accessory->original_price) }}" placeholder="0.00"
                        ...
                        <div class="space-y-2">
                            ...
                            <input name="stock" type="number" min="0" value="{{ old('stock', $accessory->stock) }}"
                                class="w-full px-4 py-3 bg-surface-container rounded-lg ring-1 ring-outline-variant/20 focus:ring-2 focus:ring-primary/20 outline-none transition-all text-sm" required>
                        </div>
                    </div>
                </div>
    ...
    {{-- Delete form lives outside the update form to avoid nested-form conflicts --}}
    <form id="delete-form" action="{{ route('admin.accessories.destroy', $accessory) }}" method="POST"
          onsubmit="return confirm('Permanently delete {{ addslashes($accessory->name) }}?')">
        @csrf @method('DELETE')
    </form>
