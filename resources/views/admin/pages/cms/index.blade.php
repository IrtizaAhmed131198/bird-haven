...
@section('title', 'CMS Pages | BirdHaven Admin')
...
<div class="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-6">
    <div>
        <h2 class="font-headline font-extrabold text-4xl text-on-surface tracking-tight">CMS Pages</h2>
        <p class="text-on-surface-variant mt-2">Manage static content pages for the frontend.</p>
    </div>
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
    <div class="md:col-span-2 bg-surface-container-low rounded-DEFAULT p-4 flex items-center gap-4">
        ...
        <div class="flex gap-2 flex-wrap" id="status-filters">
            ...
            <button data-status="published" class="status-btn inactive-filter px-3 py-1 rounded-full text-xs font-medium border transition-colors">Published</button>
            <button data-status="draft"     class="status-btn inactive-filter px-3 py-1 rounded-full text-xs font-medium border transition-colors">Draft</button>
        </div>
    </div>
    <div class="bg-surface-container-low rounded-DEFAULT p-4 flex items-center justify-between">
        <span class="text-xs font-bold uppercase tracking-wider text-on-surface-variant">Total Pages</span>
...
<div class="bg-surface-container-lowest/80 backdrop-blur-xl rounded-lg shadow-xl shadow-on-surface/5 overflow-hidden border border-white/40">
    <div class="overflow-x-auto p-4">
        <table id="cms-table" class="w-full text-left">
            <thead>
                <tr>
                    <th>Title</th>
                    <th>Slug</th>
...
$(function () {
    ...
    const table = $('#cms-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: '{{ route('admin.cms.pages.data') }}',
        ...
        columns: [
            {
                ...
                render: (data, type, row) => `
                    <div>
                        <div class="font-headline font-bold text-on-surface leading-tight">${data}</div>
                        ${row.meta_description
                            ? `<div class="text-xs text-on-surface-variant mt-0.5 line-clamp-1 max-w-[280px]">${row.meta_description}</div>`
                            : ''}
                    </div>`
            },
            {
            ...
            },
            {
                ...
                render: data => data
                    ? `<span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest bg-tertiary-container text-on-tertiary-container">
                    ...
                    : `<span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest bg-surface-container-high text-on-surface-variant">
                           <span class="material-symbols-outlined text-[13px]">edit_note</span>Draft
                       </span>`
            },
            {
