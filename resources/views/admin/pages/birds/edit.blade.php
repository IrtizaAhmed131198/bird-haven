...
@section('title', 'Edit ' . $bird->name . ' | BirdHaven Admin')
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h2 class="font-headline font-extrabold text-3xl text-on-surface">Edit Bird</h2>
            <p class="text-on-surface-variant mt-1">Editing: <span class="font-semibold text-primary">{{ $bird->name }}</span></p>
        </div>
    </div>

    <form action="{{ route('admin.birds.update', $bird) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="space-y-6">
                    ...
                    <div class="flex flex-col items-center gap-4">
                        <div class="relative">
                            <img id="image-preview"
                                 src="{{ $bird->image ? asset('uploads/images/birds/' . $bird->image) : 'https://ui-avatars.com/api/?name=' . urlencode($bird->name) . '&background=baeaff&color=005870&bold=true&size=256' }}"
                                 alt="{{ $bird->name }}"
                        ...
                        <p class="text-xs text-on-surface-variant text-center">Leave blank to keep current</p>
                    </div>
                </div>
                ...
                <div class="bg-surface-container-lowest rounded-lg shadow-sm border border-outline-variant/10 p-6">
                    ...
                    {{-- Existing images --}}
                    @php $gallery = $bird->gallery ?? []; @endphp
                    @if(!empty($gallery))
                    <div id="existing-gallery" class="grid grid-cols-3 gap-2 mb-4">
                        @foreach($gallery as $img)
                        <div class="relative group" id="gitem-{{ $loop->index }}">
                            <img src="{{ file_exists(public_path('uploads/images/birds/' . $img)) ? asset('uploads/images/birds/' . $img) : asset('assets/images/' . $img) }}"
                                 onerror="this.src='{{ asset('assets/images/default.png') }}'"
                                 class="w-full aspect-square object-cover rounded-lg">
                            <button type="button"
                                    onclick="removeGalleryImage('{{ $img }}', 'gitem-{{ $loop->index }}')"
                                    class="absolute top-1 right-1 w-6 h-6 bg-error text-white rounded-full text-xs flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow">
                    ...
                    {{-- Hidden remove inputs injected by JS --}}
                ...
                <div class="bg-surface-container-lowest rounded-lg shadow-sm border border-outline-variant/10 p-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="col-span-2 space-y-2">
                            ...
                            <input name="name" type="text" value="{{ old('name', $bird->name) }}"
                        ...
                        <div class="space-y-2">
                            ...
                            <input name="sku" type="text" value="{{ old('sku', $bird->sku) }}"
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="category_id"
                                ...
                                @foreach($categories as $cat)
                                    <option value="{{ $cat->id }}" {{ old('category_id', $bird->category_id) == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <input name="species" type="text" value="{{ old('species', $bird->species) }}"
                        ...
                        <div class="space-y-2">
                            ...
                            <input name="badge" type="text" value="{{ old('badge', $bird->badge) }}"
                        ...
                        <div class="col-span-2 space-y-2">
                            ...
                            <input name="subtitle" type="text" value="{{ old('subtitle', $bird->subtitle) }}"
                                class="w-full px-4 py-3 bg-surface-container rounded-lg ring-1 ring-outline-variant/20 focus:ring-2 focus:ring-primary/20 outline-none transition-all text-sm">
                        </div>
                        ...
                        <div class="col-span-2 space-y-2">
                            ...
                            <textarea name="description" rows="4"
                                class="w-full px-4 py-3 bg-surface-container rounded-lg ring-1 ring-outline-variant/20 focus:ring-2 focus:ring-primary/20 outline-none transition-all text-sm resize-none">{{ old('description', $bird->description) }}</textarea>
                        </div>
                ...
                <div class="bg-surface-container-lowest rounded-lg shadow-sm border border-outline-variant/10 p-6">
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        <div class="space-y-2">
                            ...
                            <input name="price" type="number" step="0.01" min="0" value="{{ old('price', $bird->price) }}"
                                class="w-full px-4 py-3 bg-surface-container rounded-lg ring-1 ring-outline-variant/20 focus:ring-2 focus:ring-primary/20 outline-none transition-all text-sm" required>
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-bold text-on-surface-variant uppercase tracking-wider">Original Price</label>
                            <input name="original_price" type="number" step="0.01" min="0" value="{{ old('original_price', $bird->original_price) }}"
                                class="w-full px-4 py-3 bg-surface-container rounded-lg ring-1 ring-outline-variant/20 focus:ring-2 focus:ring-primary/20 outline-none transition-all text-sm">
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-bold text-on-surface-variant uppercase tracking-wider">Stock *</label>
                            <input name="stock" type="number" min="0" value="{{ old('stock', $bird->stock) }}"
                                class="w-full px-4 py-3 bg-surface-container rounded-lg ring-1 ring-outline-variant/20 focus:ring-2 focus:ring-primary/20 outline-none transition-all text-sm" required>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-surface-container-lowest rounded-lg shadow-sm border border-outline-variant/10 p-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2">
                            ...
                            <input name="age" type="text" value="{{ old('age', $bird->age) }}"
                                class="w-full px-4 py-3 bg-surface-container rounded-lg ring-1 ring-outline-variant/20 focus:ring-2 focus:ring-primary/20 outline-none transition-all text-sm">
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-bold text-on-surface-variant uppercase tracking-wider">Temperament</label>
                            <input name="temperament" type="text" value="{{ old('temperament', $bird->temperament) }}"
                                class="w-full px-4 py-3 bg-surface-container rounded-lg ring-1 ring-outline-variant/20 focus:ring-2 focus:ring-primary/20 outline-none transition-all text-sm">
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-bold text-on-surface-variant uppercase tracking-wider">Color</label>
                            <input name="color" type="text" value="{{ old('color', $bird->color) }}"
                                class="w-full px-4 py-3 bg-surface-container rounded-lg ring-1 ring-outline-variant/20 focus:ring-2 focus:ring-primary/20 outline-none transition-all text-sm">
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-bold text-on-surface-variant uppercase tracking-wider">Wingspan (cm)</label>
                            <input name="wingspan_cm" type="number" min="0" value="{{ old('wingspan_cm', $bird->wingspan_cm) }}"
                                class="w-full px-4 py-3 bg-surface-container rounded-lg ring-1 ring-outline-variant/20 focus:ring-2 focus:ring-primary/20 outline-none transition-all text-sm">
                        </div>
                        <div class="col-span-2 space-y-2">
                            <label class="text-xs font-bold text-on-surface-variant uppercase tracking-wider">Habitat</label>
                            <textarea name="habitat" rows="2"
                                class="w-full px-4 py-3 bg-surface-container rounded-lg ring-1 ring-outline-variant/20 focus:ring-2 focus:ring-primary/20 outline-none transition-all text-sm resize-none">{{ old('habitat', $bird->habitat) }}</textarea>
                        </div>
                        <div class="col-span-2 space-y-2">
                            <label class="text-xs font-bold text-on-surface-variant uppercase tracking-wider">Nutrition</label>
                            <textarea name="nutrition" rows="2"
                                class="w-full px-4 py-3 bg-surface-container rounded-lg ring-1 ring-outline-variant/20 focus:ring-2 focus:ring-primary/20 outline-none transition-all text-sm resize-none">{{ old('nutrition', $bird->nutrition) }}</textarea>
                        </div>
                        <div class="col-span-2 space-y-2">
                            ...
                            <textarea name="social" rows="2"
                                class="w-full px-4 py-3 bg-surface-container rounded-lg ring-1 ring-outline-variant/20 focus:ring-2 focus:ring-primary/20 outline-none transition-all text-sm resize-none">{{ old('social', $bird->social) }}</textarea>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="space-y-4">
                        <div class="space-y-2">
                            ...
                            <textarea name="habitat_guide" rows="3"
                                class="w-full px-4 py-3 bg-surface-container rounded-lg ring-1 ring-outline-variant/20 focus:ring-2 focus:ring-primary/20 outline-none transition-all text-sm resize-none">{{ old('habitat_guide', $bird->habitat_guide) }}</textarea>
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-bold text-on-surface-variant uppercase tracking-wider flex items-center gap-1.5">
                            ...
                            <textarea name="nutrition_guide" rows="3"
                                class="w-full px-4 py-3 bg-surface-container rounded-lg ring-1 ring-outline-variant/20 focus:ring-2 focus:ring-primary/20 outline-none transition-all text-sm resize-none">{{ old('nutrition_guide', $bird->nutrition_guide) }}</textarea>
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-bold text-on-surface-variant uppercase tracking-wider flex items-center gap-1.5">
                            ...
                            <textarea name="social_guide" rows="3"
                                class="w-full px-4 py-3 bg-surface-container rounded-lg ring-1 ring-outline-variant/20 focus:ring-2 focus:ring-primary/20 outline-none transition-all text-sm resize-none">{{ old('social_guide', $bird->social_guide) }}</textarea>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    // Gallery — remove existing image
    function removeGalleryImage(filename, itemId) {
        // Hide the thumbnail
        const el = document.getElementById(itemId);
        if (el) el.remove();

        // Inject hidden input so controller knows to remove it
    ...
    // Gallery — preview newly selected files
    document.getElementById('gallery-input').addEventListener('change', function () {
        ...
        // Build a new DataTransfer to hold only kept files
